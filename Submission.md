...
system actually works. [README.md](README.md) is how to run it. [ENGINEERING.md](ENGINEERING.md)
is the long-form reasoning behind each decision. This is the walkthrough.
...
```
question -> nomic-embed-text -> Chroma (cosine) -> top 5 chunks -> llama3.2 -> grounded answer
```
...
| Keyword search alone is not sufficient | — | Retrieval is vector-based, not keyword-based. Asking *"what must an organization do for automated system account management?"* retrieves control `AC-2(1)` at 0.8420 without sharing those words with the source text |
...
| Preserve conversational context (**bonus**) | — | **Not built.** See section 9 |
...
| Asked for | Provided |
|---|---|
| Complete source code | `src/csrs/` (2164 lines), `frontend/` (2436 lines), `tests/` (133 passing) |
...
3. **Search the vector store.** `ChunkStore.search()` asks Chroma for the `k` nearest chunks
   by cosine distance. Default `k` is 5.
4. **Convert distance to similarity.** Chroma returns a distance; `1.0 - distance` gives the
   ...
   metadata stored with it: document name, page, section breadcrumb, control ID.
...
| Chunks sent to the model | 5 | |
| Retrieval candidate pool | 20 | Reserved for the reranker; see section 9 |
...
**The test suite proves the offline claim.** The 133 offline tests run with Ollama pointed at
...
**An evaluation golden set exists.** 48 hand-authored question/answer pairs across five
categories — exact control lookup, semantic paraphrase, cross-document, out-of-scope (must
refuse), and all five of the specification's own example questions. A validator checks that
every expected answer still resolves to a real chunk in the index.
...
**Retrieval is dense-only.** Semantic similarity with no BM25 keyword hybrid and no reranker.
The dependencies for both are declared but unused. The golden set is built; the metrics
harness that would measure Recall@10, MRR and nDCG is the next task. Until it runs, retrieval
quality is demonstrated by example rather than proven by number.
...
Work on the first two items is in progress. `ROADMAP.md` has the task breakdown.
