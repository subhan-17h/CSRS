...
\begin{frame}[plain]
  ...
  \vspace{1.6em}
  \begin{tabular}{@{}ll@{}}
    {\color{soft}\small Intern} & \small Muhammad Subhan Amir \\
    {\color{soft}\small Student ID} & \small BSCS23016 \\
    {\color{soft}\small Institution} & \small Information Technology University \\
...
% --- 3 ---------------------------------------------------------------
\begin{frame}{Success would mean four things}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \textbf{\color{accent}Grounded answers}\\[0.25em]
      {\small Respond from retrieved standards evidence, cite its page and section, and
      refuse when the corpus cannot support an answer.}
      \vspace{1.0em}

      \textbf{\color{accent}Local core}\\[0.25em]
      {\small Parse, embed, retrieve and generate on the host after models are warmed.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{\color{accent}A usable product}\\[0.25em]
      {\small Deliver the required Streamlit interface and a richer React + FastAPI path,
      both driving the same pipeline.}
      \vspace{1.0em}

      \textbf{\color{accent}Measured, reproducible work}\\[0.25em]
      {\small Evaluate answer quality on a fixed question set, then preserve every alert
      experiment request, passage and verdict for replay.}
    \end{column}
  \end{columns}
  \keytake{Success is not ``the model produced text'' --- it is evidence, restraint, usability and measurement.}
\end{frame}
...
\begin{frame}{Two interfaces, one pipeline}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{center}\includegraphics[width=\textwidth,height=0.58\textheight,keepaspectratio]{figures/ui_web_answer.png}\end{center}
    ...
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{center}\includegraphics[width=\textwidth,height=0.58\textheight,keepaspectratio]{figures/ui_streamlit.png}\end{center}
...
% --- 11 --------------------------------------------------------------
\begin{frame}{Live demo: one question, four checks}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.34\textwidth}
      {\small
      \textbf{\color{accent}1. Configure}\\
      Choose the installed model and retrieval settings.
      \vspace{0.65em}

      \textbf{\color{accent}2. Ask}\\
      Submit a standards question in plain language.
      \vspace{0.65em}

      \textbf{\color{accent}3. Observe}\\
      Watch retrieval stages and answer tokens stream.
      \vspace{0.65em}

      \textbf{\color{accent}4. Verify}\\
      Expand the source card, then open the cited document.}
    \end{column}
    \begin{column}{0.62\textwidth}
      \begin{center}
        \includegraphics[width=\textwidth,height=0.63\textheight,keepaspectratio]{figures/ui_web_answer.png}
      \end{center}
      {\footnotesize\color{soft}The useful demo is not the prose alone; it is the path from
      a question to inspectable evidence.}
    \end{column}
  \end{columns}
\end{frame}
...
% --- 15 --------------------------------------------------------------
\begin{frame}{Week 3 --- define severity before ranking it (Days 9--10)}
  \leavevmode
  {\footnotesize
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.10\textwidth}
    ...
    \toprule
    \textbf{When} & \textbf{Date} & \textbf{Decision or delivery} \\
    \midrule
    Day 9 & 3 Aug & Wrote the nine-criterion severity rubric; priority is a clue and frequency is volume, not danger. \\
    Research & 4--5 Aug & Made the task gradeable without human labels and decided how the visible priority could remain a clue, not the answer. \\
    Day 10 & 6 Aug & Added SP 800-53r5, the 50-alert ranking runner and its report builder. \\
    Research & 7 Aug & Set anchors 1, 3 and 5, with a mismatch only beyond one step. \\
    \bottomrule
  \end{tabular}}
  \keytake{Week 3 made severity explicit and machine-checkable.}
\end{frame}

% --- 16 --------------------------------------------------------------
\begin{frame}{Week 4 --- production runs expose the real errors (Days 11--14)}
  \leavevmode
  {\footnotesize
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.10\textwidth}
    ...
    \toprule
    \textbf{Day} & \textbf{Date} & \textbf{What changed} \\
    \midrule
    Day 11 & 10 Aug & Shared anchor logic, mismatch analysis and a severity judge entered the report. \\
    Day 12 & 11 Aug & Hosted rank/judge chain gained rate limits and quota-safe resume; a smoke run caught the required reasoning flag. \\
    Day 13 & 12 Aug & v1 ranked all 50 alerts: 21 exact, 23 mismatches, self-judge mean 0.586. \\
    Day 14 & 13 Aug & v2 withheld the answer key, split model families and indexed 4,022 rules: 32 exact, 4 mismatches. \\
    Research & 14 Aug & The remaining rule-match errors traced to generic one-line rule documents. \\
    \bottomrule
  \end{tabular}}
\end{frame}

% --- 17 --------------------------------------------------------------
\begin{frame}{Week 5 --- improve the corpus, keep the regression (Days 15--16)}
  \leavevmode
  {\small
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.10\textwidth}
    ...
    \toprule
    \textbf{When} & \textbf{Date} & \textbf{Outcome} \\
    \midrule
    Day 15 & 15 Aug & Completed the verified history, committed evaluation artefacts and built the report figure pipeline. \\
    Research & 17--20 Aug & Prepared 4,017 detailed rule documents; 3,945 carry full published documentation fields. \\
    Day 16 & 21 Aug & v3 reached 40 correct rule matches, none wrong and 10 abstentions --- while exact severity matches fell from 32 to 29. \\
    \bottomrule
  \end{tabular}}
  \keytake{The internship closed on the measured v3 outcome, not on a prompt tuned after seeing it.}
\end{frame}
...
% --- 29 --------------------------------------------------------------
\begin{frame}{Representative alert: when strong evidence over-corrects}
  \lede{Alert 354 is a documented CVSS-10.0 spot-check and a useful example of the ranking failure mode.}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.30\textwidth}
      \textbf{\color{accent}Ground-truth anchor}\\[0.35em]
      {\small Snort Priority 2 maps to severity rank \textbf{3}.}
    \end{column}
    \begin{column}{0.30\textwidth}
      \textbf{\color{accent}Model decision}\\[0.35em]
      {\small CVSS-10.0 evidence pulled the model to rank \textbf{1}: two steps more severe.}
    \end{column}
    \begin{column}{0.30\textwidth}
      \textbf{\color{accent}Independent verdict}\\[0.35em]
      {\small Judge score \textbf{0.00}; the departure was rejected, not treated as useful refinement.}
    \end{column}
  \end{columns}
  \vspace{1.2em}
  \noindent\colorbox{accenttint}{%
    \begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
      \vspace{5pt}
      \small\color{ink}
      \textbf{What the case teaches.} Better retrieval can expose genuine technical danger
      and still make the end decision worse when the prompt gives that evidence too much
      freedom to override its calibration anchor.
      \vspace{5pt}
    \end{minipage}}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}
        ...
        \item {\small \textbf{343 offline tests}, Ruff clean.}
...
\begin{frame}{Deliverables, and the next experiment}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      ...
      {\small
      \begin{itemize}\setlength{\itemsep}{1pt}
        \item Source repository, 117 verified commits
...
\begin{frame}[plain]
  \vspace{1.0em}
  {\color{accent}\Large\bfseries Three takeaways}\par
  \vspace{0.9em}

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.30\textwidth}
      \textbf{\color{accent}01}\\[0.25em]
      {\small A local, cited RAG system can be useful with small models when its evidence
      path is explicit.}
    \end{column}
    \begin{column}{0.30\textwidth}
      \textbf{\color{accent}02}\\[0.25em]
      {\small Metrics shape engineering decisions; a flattering target is worse than no
      target because it rewards the wrong behavior.}
    \end{column}
    \begin{column}{0.30\textwidth}
      \textbf{\color{accent}03}\\[0.25em]
      {\small Richer evidence is not automatically better. Measure every output it can
      change, and keep the regressions.}
    \end{column}
  \end{columns}

  \vfill
  {\color{rulegrey}\rule{\textwidth}{0.8pt}}\\[0.8em]
  {\color{accent}\fontsize{24}{28}\selectfont\bfseries Questions?}\par
  \vspace{0.25em}
  {\color{soft}\small Muhammad Subhan Amir \quad BSCS23016 \quad Information Technology University}
\end{frame}
